...
EOF
    ...
    if curl -fL -# --connect-timeout 60 -o /tmp/web_template.zip "https://github.com/rumicho8/Nginx-3DCEList/archive/refs/heads/main.zip"; then
        ...
        rm -rf /tmp/web_template.zip /tmp/Nginx-3DCEList-main /tmp/*-main 2>/dev/null
    else
        echo -e "${C_RED}✖ 模板下载失败，网络超时或该仓库不存在。${C_RESET}"
    fi
    echo -e "${C_BLUE}----------------------------------------------------${C_RESET}"
...
module_install_xray_core() {
    ...
    local latest_api="https://api.github.com/repos/XTLS/Xray-core/releases/latest"
    
    local latest_json
    latest_json=$(curl -sL --connect-timeout 10 "$latest_api")
    
    local zip_url
    zip_url=$(echo "$latest_json" | jq -r ".assets[] | select(.name==\"$zip_name\") | .browser_download_url" 2>/dev/null)
    
    [[ -z "$zip_url" || "$zip_url" == "null" ]] && log_err "无法解析 Xray 下载地址，请检查网络连通性或 GitHub 接口状态。"
    
    echo -e "${C_BLUE}------------------- 核心下载进度 -------------------${C_RESET}"
    if ! curl -fL -# --connect-timeout 10 -o "$zip_name" "$zip_url"; then
        log_err "核心包下载失败！"
    fi
    echo -e "${C_BLUE}----------------------------------------------------${C_RESET}"

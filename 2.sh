...
# =========================================================
# 模块 8：无痕模式 (SSH ForceCommand 级别接管)
# =========================================================
module_setup_stealth() {
    case "${GLOBAL_ENABLE_STEALTH}" in
        [yY][eE][sS]|[yY])
            log_info "正在注入系统级 SSH 强力清痕引擎 (ForceCommand)..."

            # -----------------------------
            # 清理脚本（优化版）
            # -----------------------------
            cat >/usr/local/bin/ssh_session_clean <<'EOF'
#!/bin/bash

# 只处理真实用户
for u in $(awk -F: '$3 >= 1000 && $7 !~ /(nologin|false)/ {print $1}' /etc/passwd); do
    home=$(getent passwd "$u" | cut -d: -f6)
    [ -d "$home" ] && rm -f "$home/.bash_history" "$home/.zsh_history" 2>/dev/null
done
...
exit 0
EOF
            chmod 700 /usr/local/bin/ssh_session_clean

            # -----------------------------
            # wrapper（修正空格 Bug 版）
            # -----------------------------
            cat >/usr/local/bin/ssh_wrapper <<'EOF'
#!/bin/bash
...
if [ -n "$SSH_ORIGINAL_COMMAND" ]; then
    case "$SSH_ORIGINAL_COMMAND" in
        scp*|rsync*|sftp*)
            # 修复：加上 eval 以防止含空格的文件路径在 exec 时被意外拆分
...
/usr/local/bin/ssh_session_clean >/dev/null 2>&1

exit $EXIT_STATUS
EOF
            chmod 755 /usr/local/bin/ssh_wrapper
            ...
            grep -q "ForceCommand /usr/local/bin/ssh_wrapper" "$SSHD" || \
            echo "ForceCommand /usr/local/bin/ssh_wrapper" >> "$SSHD"

            if grep -q "^LogLevel" "$SSHD"; then
                sed -i 's/^LogLevel.*/LogLevel ERROR/' "$SSHD"
            else
                echo "LogLevel ERROR" >> "$SSHD"
            fi

            systemctl restart sshd 2>/dev/null || systemctl restart ssh 2>/dev/null
            log_ok "SSH 强力无痕模式已全局配置成功。"
            ;;
        *) log_info "未启用无痕模式，跳过。" ;;
    esac
}
...
while true; do
    ...
    case $OPT in
        ...
        2)
            ...
            # --- SSH ForceCommand 相关清理与还原 (已修复 local 报错) ---
            SSHD_CONF="/etc/ssh/sshd_config"
            if [ -f "$SSHD_CONF" ]; then
                sed -i '/ForceCommand \/usr\/local\/bin\/ssh_wrapper/d' "$SSHD_CONF"
                sed -i 's/^LogLevel ERROR/LogLevel INFO/' "$SSHD_CONF"
                systemctl restart sshd 2>/dev/null || systemctl restart ssh 2>/dev/null
            fi
            rm -f /usr/local/bin/ssh_wrapper /usr/local/bin/ssh_session_clean

...
# =========================================================
readonly SCRIPT_VERSION="v12.16 (自带路由托底 + 防死锁 + 证书分流完美版)"
...
module_get_inputs() {
    ...
    # 【修复项 1】：动态分流，防止无参数时被强制转给 ZeroSSL
...
module_issue_cert() {
    ...
    if [[ ! -f "/etc/nginx/ssl/${domain}_ecc.cer" ]]; then
        ...
        if [[ "$api" == "standalone" ]]; then
            systemctl stop nginx >/dev/null 2>&1 # 必须停掉 Nginx 让出 80 端口
            # 【修复项 2】：去掉了结尾写死的 --server letsencrypt
            /root/.acme.sh/acme.sh --issue -d "$domain" --standalone --keylength ec-256 $GLOBAL_CERT_MODE 2>&1 | tee -a "$LOG_FILE"
        else
            # 【修复项 2】：去掉了结尾写死的 --server letsencrypt
...
EOF
    ...
    # 【修复补丁】：掐断早产重启，根治 Systemd 死锁，修改 UI 宣告
...
EOF
    ...
    # 【新增宣告】：在全局唯一的点火动作（update-dat.sh 结尾的 restart）之后进行宣告
...
module_show_result() {
    ...
    # 【修复项 3】：消除 UI 误判 Bug。必须精准匹配 --staging，否则正式证书也会弹警告

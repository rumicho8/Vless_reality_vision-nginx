...
# =========================================================
readonly SCRIPT_VERSION="V12.8 强迫症终极满足版"
readonly LOG_FILE="/var/log/xray_install.log"
...
module_setup_automation() {
    log_info "配置路由规则原子更新机制 (update-dat.sh)..."
...
EOF
    ...
    (crontab -l 2>/dev/null | grep -v "update-dat.sh" ; echo "0 3 * * 1 $SCRIPT_DIR/update-dat.sh > /dev/null 2>&1") | crontab -
    log_ok "自动化任务编排完成。"
}
...
module_show_result() {
    local domain=$1
    clear
    log_ok "部署/更新圆满完成！所有底层日志已保存至 $LOG_FILE"
...
while true; do
    ...
    case $OPT in
        ...
        2)
            ...
            rm -rf "$XRAY_CONF_DIR" "$XRAY_SHARE_DIR" "$SCRIPT_DIR" /etc/nginx/ssl /root/.acme.sh "$LOG_FILE"
